...
- Type guards that only leveraged the `typeof` operator to test value type. Those were
moved to [@infra-blocks/toolkit](https://github.com/infra-blocks/ts-toolkit). Those are:
  - `isBigint`
  - `isBoolean`
  - `isFunction`
  - `isNumber`
  ...
  - `isSymbol`
  - `isUndefined`
- Predicates that can be found in [es-toolkit](https://github.com/toss/es-toolkit).
We'll defer to external lib for now. Those are:
  - `isNonNullable`
  - `isNull`
  - `isPlainObject`
  - `isPrimitive`
...
- `RequiredKeys`, the complement to `OptionalKeys`.

## [0.30.0] - 2026-03-02

### Added

- `OptionalKeys` to extract optional keys from a type.
...
- `isNonNullable`, which assesses whether a value can be assigned to `NonNullable<T>` or
not.
...
- The `Phantom` type to help keep track of phantom types.

### Changed

- `unreachable` now throws an `AssertionError` if called.

## [0.26.0] - 2026-01-21

### Added

- The `trusted` function, which type casts a value. Useful when the return type
is already known and a type cast is necessary.

## [0.25.0] - 2026-01-15

### Added

- `WithPartial` and `WithRequired` utility types. Those complement the functionalities
of the built-in `Partial` and `Required` by allowing specific field selections. Where
`Partial<T>` turns every field partial, `WithPartial<T, K extends keyof T>` allows the
user to specify *which* fields should be made partial, leaving the others untouched.
Ditto for `WithRequired`.

## [0.24.0] - 2025-12-29

### Changed

- The error type associated to the `ErrorHandler` now defaults to `unknown`. Although
`Error` was more convenient and is idiomatic JS code, `unknown` is more correct as JS allows
to throw anything, not just error types, and we should default to allow for the behavior
the platform allows.
...
- The `TemplateExpression` type representing types that can be used as a template expression.
This is useful in client code that only wants to dispatch to a template literal. It should
be noted that template literal don't compile with a symbols, and throw `TypeError` when
one is provided. Therefore, `symbols` are excluded from the union.
...
[0.24.0]: https://github.com/infra-blocks/ts-types/compare/v0.23.0...v0.24.0
[0.23.0]: https://github.com/infra-blocks/ts-types/compare/v0.22.0...v0.23.0
